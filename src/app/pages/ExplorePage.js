...
const ExplorePage = (props) => {
    ...
    useEffect(() => {
        db.collection('posts').where("user", "!=", context.User.data).onSnapshot((snapshot) => {
                        ...
                        PostData.map((channel, index) => {
                            return(
                                channel.data.topic.toLowerCase().includes(Search.toLowerCase()) && <Card 
                                    ...
                                    avatar={channel.data.user.photo}
                                    uid={channel.data.user.uid}
                                    channelName={channel.data.channelName}
                                    teacherName={channel.data.user.username}
                                />
                            )
                        })
                    )
